...
def fitness_func(ga_instance, solution, solution_idx):
    ...
    if l_low >= l_high: return 1e-6
    if c_safe >= c_danger: return 1e-6
    if l_low < 0.1 or l_high > 1.9: return 1e-6
    ...
    for _, row in sim_data.iterrows():
        ...
        # C. Apply Decision
        if decision > 0.6: current_replicas += 1
        elif decision < -0.6 and current_replicas > 1: current_replicas -= 1
