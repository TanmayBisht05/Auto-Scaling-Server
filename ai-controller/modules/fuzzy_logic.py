...
class FuzzyBrain:
    def __init__(self, ga_params=None):
        ...
        # LOAD shapes
        self.load_ratio['low'] = fuzz.trimf(self.load_ratio.universe, [0, 0, L_LOW])
        self.load_ratio['medium'] = fuzz.trimf(self.load_ratio.universe, [0, L_LOW, L_HIGH])
        self.load_ratio['high'] = fuzz.trimf(self.load_ratio.universe, [L_LOW, L_HIGH, 2.0])

        # CPU shapes
        self.cpu['safe'] = fuzz.trimf(self.cpu.universe, [0, 0, C_SAFE])
        self.cpu['stable'] = fuzz.trimf(self.cpu.universe, [0, C_SAFE, C_DANGER])
        self.cpu['critical'] = fuzz.trimf(self.cpu.universe, [C_SAFE, C_DANGER, 100])

        # OUTPUT shapes (Fixed)
        self.action['scale_down'] = fuzz.trimf(self.action.universe, [-1, -1, 0])
        self.action['hold']       = fuzz.trimf(self.action.universe, [-0.5, 0, 0.5])
        self.action['scale_up']   = fuzz.trimf(self.action.universe, [0, 1, 1])

        # --- 3. RULES ---
        rule1 = ctrl.Rule(self.load_ratio['high'] | self.cpu['critical'], self.action['scale_up'])
        rule2 = ctrl.Rule(self.load_ratio['low'] & self.cpu['safe'], self.action['scale_down'])
        rule3 = ctrl.Rule(self.load_ratio['medium'] | self.cpu['stable'], self.action['hold'])
